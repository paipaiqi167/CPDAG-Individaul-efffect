...
hilma <- function(Y, G, S, mediation_setting = 'incomplete', tuning_method = 'uniform', lam_list = NA,
  ...
  if (center == TRUE) {
    ...
    Y = Y - mean(Y)
    S=apply(S, 2, function(x) x-mean(x))
    G=apply(G, 2, function(x) x-mean(x))
  ...
  if (mediation_setting == 'incomplete') {
    ...
    # Cross.L = cv.glmnet(X,Y,nfolds = 10,type.measure = 'default')
    # result_lasso = glmnet(X,Y,lambda = Cross.L$lambda.min)
    # 
    # result_scalreg = scalreg(round(X,2),round(Y,2))
    # alpha_hat = result_scalreg$co
    # sigma1_hat = result_scalreg$hsigma
    # 
    # alpha_hat = coef(result_lasso)[-1]
    # Y_hat = predict( result_lasso,X) 
    # sigma1_hat = mean((Y-Y_hat)^2)
    # 
    # if(sigma1_hat > 10) {
    #   
    #   Cross.L = cv.glmnet(round(X,2),round(Y,2),nfolds = 10,type.measure = 'default')
    #   result_lasso = glmnet(round(X,2),round(Y,2),lambda = Cross.L$lambda.min)
    #   
    #   alpha_hat = coef(result_lasso)[-1]
    #   Y_hat = predict( result_lasso,round(X,2)) 
    #   sigma1_hat = mean((Y-Y_hat)^2)
    #   
    # }
    # 
    ...
    if(norm(beta_part1)==0)
    {
      cat('XXXXXXXXXX \n')
    }
    cat('This is beta_part1','\n')
    print(beta_part1)
    ...
    Sigma_11 = Cov_part1[1:q,1:q] + sigma2_hat*sigma_SS_hat_inverse
    ...
    if (q > 1) {
      ...
      infer_out$upper = beta_hat + 1.96*sqrt(diag (sigma_beta_hat) /n)
      infer_out$lower = beta_hat - 1.96*sqrt(diag (sigma_beta_hat) /n)
      
    } else if (q == 1) {
      teststat_beta = beta_hat/sqrt(sigma_beta_hat /n)
      teststat_alpha1 = alpha1_hat/sqrt( sigma_alpha1_hat/n)
      infer_out$upper = beta_hat + 1.96*sqrt(sigma_beta_hat /n)
      infer_out$lower = beta_hat - 1.96*sqrt(sigma_beta_hat /n)
    }
    ...
    infer_out$alpha_original_hat = alpha_hat[p+1]
